...
class Game2048Env(gym.Env):  # directions 0, 1, 2, 3 are up, right, down, left
    ...
    def __init__(self, board_size=4):
        ...
        self.max_illegal = 100  # max number of illegal actions
        self.num_illegal = 0
    ...
    def step(self, action):
        ...
        try:
            ...
            self.score += score
            self.best_score = np.max([self.score, self.best_score])
            # print(score)
            # print(self.Matrix)
            # print(2 ** (self.w * self.h))
            # assert score <= 2 ** (self.w * self.h)
            self.add_tile()
            done = self.checkIfLose()
            # done = self.isend()
            reward = float(score)
    ...
    def checkIfLose(self):
        items = self.Matrix.flatten()
        lose = True
        np_items = np.array(items).reshape((self.size, self.size))
        for i in range(self.size):
            for j in range(self.size):
                ...
                i_tmp = i - 1
                if 0 <= i_tmp < self.size:
                    if items[self.size * i + j] == items[self.size * i_tmp + j]:
                        return False
                ...
                j_tmp = j - 1
                if 0 <= j_tmp < self.size:
                    if items[self.size * i + j] == items[self.size * i + j_tmp]:
                        return False
